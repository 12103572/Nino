...
class map2048():
    ...
    def reset(self):
        ...
        # self.data = [[x + 4 * y for x in range(self,_col)]
    ...
    # 填充2到空闻之，如果填度成功返回Ture,如果已满，则返回False,
    def fill2(self):
    ...
    def left(self):
        # mobeflag 是否成功移动数字标志位，如果有移动则为真值，原地图不变则为假值
        moveflag = False
        # 将所有数字向上移动来填补左侧空格
            ...
            for r in self.data:
                for c in range(self.__col - 1):
                    if 0 == r[c]:
                        ...
                        r[c] = r[c + 1]
                        r[c + 1] = 0
        #　判断是否发生碰撞，如果有碰撞则合并，合并结果考上，下面填充空格
        for r in self.data:
            for c in range(self.__col - 1):
                if r[c] == r[c + 1]:
                    moveflag = True
...
root = TK()
root.title('2048')
# root.iconbitmap('./favicon.ico)  # 48x48 ic0 bitmap

frame = Frame(root,width=300,height=300,bg=game_bg_color)
frame.grid(sticky=N+E+W+S)
# 按键事件见：http://blog.csdn.net/qq＿25600055／article/details/46942035
# 设置焦点能接受按键事件
frame.focus_set()
...
# 见　:http://blog.scdn.net/wjciayf/article/details/50550947
